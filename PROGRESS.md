...
**Last updated:** 2026-06-28 (Phase 1 hardening continues: #19 Pronunciation Score phoneme-distance endpoint shipped on `feat/issue-19-pronunciation-score-endpoint`; 448/448 tests green; #25 + #30 + #28 + #44 + #46 in flight on the dep-ordered Season 2 queue)

## Current focus

**Season 2 in flight.** #19 closed (Pronunciation Score phoneme-distance endpoint + ASR-bias free-form path + drill per-phoneme UI + 10-utterance calibration set); #25 + #30 + #28 + #44 + #46 are the next picks. The repo now has 448 tests, typecheck/lint/build all green, two seeded Units-worth of A0 content, and the full Voice Loop / SRS / Proficiency / Affective Filter / Conversational Practice / LLM Difficulty Pipeline runtime. What remains is content, persistence, real-world audio capture/playback, NFRs, and the §10 spec sign-off.
...
**Test count:** 391 → **405** → **448** (+14 anchor-routing property tests, +placement page + dashboard + profile tests, +43 pronunciation tests).

## In progress

- **#19** Pronunciation Score phoneme-distance endpoint — committed on `feat/issue-19-pronunciation-score-endpoint`, awaiting PR. Test count +43.
...
**Phase 1 — runtime integration (1 remaining of 4)**
