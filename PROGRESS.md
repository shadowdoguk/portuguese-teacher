...
- **#41 — Expand CEFR level vocabulary fixture** — branch `feat/issue-41-vocab-fixture`. Sub-levels A2.1/A2.2/B1.1/B1.2 + A1→A2 and A2→B1 corpora. 191/191 tests pass. PR pending commit.
- **#40 — Wire `generateAndRerankTurn` into the voice-loop API route** — branch `feat/issue-40-rerank-orchestrator`. Tier 1+2 → rerank; Tier 3 → runTurn; `ENABLE_RERANK_PATH` feature flag; telemetry log. 178/178 tests pass. PR pending commit.
- **#7 — Conversational Practice UI + scenario library** — branch `feat/issue-7-conversational-practice`. 30 pt-PT scenarios. Follow-ups filed: #44-#48. PR pending commit.
