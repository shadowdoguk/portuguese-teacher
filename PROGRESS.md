...
**Next dep-ordered item:** #41 (Expand CEFR level vocabulary fixture) — depends on #6.

## In progress

- **#40 — Wire `generateAndRerankTurn` into the voice-loop API route** — branch `feat/issue-40-rerank-orchestrator` (off `feat/issue-6-llm-difficulty-pipeline`). `src/lib/voice-loop/rerank-orchestrator.ts` ships the path-selection helper (`chooseVoiceLoopPath`) and structured-telemetry emitter (`buildRerankTelemetry`). API route `src/app/api/voice-loop/turn/route.ts` now branches: Tier 1+2 → `generateAndRerankTurn`; Tier 3 → `runTurn`. Feature flag `ENABLE_RERANK_PATH` (`0`/`1`/unset; unset defaults to on in production, off in mock). Live mode requires MiniMax creds (covered by #42); mock mode uses `buildMockRerankLlm`. Dialect pinned to `pt-PT` per ADR-0003; Learner Level defaults to `A0` until #9 lands. 11 new tests in `src/test/rerank-orchestrator.test.ts`. 178/178 tests pass; typecheck/lint/build green. PR pending commit.
