...
**Last updated:** 2026-06-29 (Session 5 — rebase of `feat/issue-19-pronunciation-score-endpoint` onto main complete; `VoiceLoopStage` extended to include `"pronunciation"`; 681/681 tests + 9/9 axe + `pnpm perf:budget` clean + `next build` clean; PR pending; #38 ASR LM biasing is the next pick)
...
- **#19** Pronunciation Score phoneme-distance endpoint — rebased onto current main (one PROGRESS.md docs conflict resolved by adopting main's version + fresh "Session 5" entry), `VoiceLoopStage` enum extended to include `"pronunciation"` to fix the `withLatencyMetric` → `voice_loop_latency` event type narrowing. All checks green on the branch (681/681 tests, 9/9 axe, `pnpm perf:budget` clean, `next build` clean); PR pending. Closes #19, unblocks #37.
