...
| **Placement Lesson** | A single adaptive Lesson administered at sign-up when the Learner self-assesses above A0. Confirms or revises the Learner's starting Unit. Composed of 8–12 Placement Items; terminates with a Placement Outcome (per-skill mastery + recommended starting Unit) and persists a Placement Attempt record. |
| **Placement Item** | A single adaptive question/prompt presented to the Learner during a Placement Lesson. Skill ∈ {listening, reading, speaking}; kind ∈ {listening, reading, free-response}; one of 8–12 per Placement Lesson. |
| **Placement Outcome** | The algorithm's recommendation at the end of a Placement Lesson: per-skill mastery scores, overall score, recommended starting Unit and Level, and a rationale string. The Learner may Accept the recommendation or one-step self-correct to a different Unit in the same Level. |
| **Placement Attempt** | The persisted record of one Placement Lesson run: `id`, `learnerId`, `attemptedAt`, `selfAssessedLevel`, `score`, `recommendedStartUnitId`, `confirmedStartUnitId`, optional `notes`. A0 self-assessments do **not** create a Placement Attempt — A0 skips the Placement Lesson and starts at the entry Unit. |
