...
| **Scenario-to-Unit DB wiring beyond A0** | Only the 4 A0 scenarios are in the DB today; A1/A2/B1 Unit seeds have empty `scenarios: []`. The in-memory library covers all 100 scenarios. | v1.1 wires 15-20 library scenarios to seeded A1/A2/B1 Units. |
...
- [x] Test suite: 880/880 unit + 9/9 axe + 13 E2E + LHCI on `main` all green at GA cut.
...
- [ ] LGPD Art. 37 record of processing activities filed (Brazil data; pt-PT-only v1 has no Brazilian region but the file is required for the future pt-BR launch).
